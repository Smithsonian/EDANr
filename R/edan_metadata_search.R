...
edan_metadata_search <- function(query = "*:*", fqs = NA, AppID = NA, AppKey = NA, rows = 10, start = 0, returnjson = FALSE){
  ...
  QueryParameters <- paste0("q=", URLencode(query), "&rows=", rows, "&start=", start, "&facet=true")
  ...
  #API url
  api_url <- paste0('metadata/v2.0/metadata/search.htm?', QueryParameters)
